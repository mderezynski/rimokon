...
namespace MPX
{
    ...
    {
        boost::shared_ptr< ::MPXPD::IShell> ctrl = services->get< ::MPXPD::IShell>("mpx-service-shell") ; 
            ...
            if( bus )
            {
                ...
                if( m_mmkeys_dbusproxy )
                {
                    ...
                    if( error == NULL )
                    {
                        ...
                        g_printf("%p", ctrl->get_widget()) ;

                        mWindowFocusConn = dynamic_cast<Gtk::Window*>(ctrl->get_widget())->signal_focus_in_event().connect( sigc::mem_fun( *this, &MMKeys::window_focus_cb ) );
                    }
