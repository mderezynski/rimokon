...
*/
    ...
    )
    {
        ...
        try{
            ...
            SubmissionInfo info ( m.artist, m.title, time(NULL) ) ;

            info.setTrackLength( m.time ) ;
            info.setTrackNr( m.tracknr ) ;
            info.setAlbum( m.album ) ; 

            if( m.track_mpxid.length() != 0 ) 
            {
                info.setMusicBrainzId( m.track_mpxid ) ; 
            }
