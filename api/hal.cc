...
namespace MPX
{
        ...
        HAL::volume_register( const Volume& volume )
    {
        ...
        } catch( SQL::SqlConstraintError & cxe )
        {
            ...
            gint64 id_old = boost::get<gint64>(rv[0]["id"]) ;
    
            m_SQL->exec_sql(mprintf("DELETE FROM hal_volumes WHERE id = '%lld'", id_old)) ;
            ...
            m_SQL->exec_sql(mprintf("UPDATE hal_volumes SET id = '%lld' WHERE id = '%lld'", id_old, id)) ;
        ...
        g_message( "ID for '%s':'%s' is [%lld]", volume.volume_udi.c_str(), volume.device_udi.c_str(), id ) ;
